"""Test if just clearing the cache reference (no explicit dealloc) fixes the leak."""
...
class mallinfo2_t(ctypes.Structure):
    _fields_ = [
        ...
        ("usmblks", ctypes.c_size_t), ("fsmblks", ctypes.c_size_t), ("uordblks", ctypes.c_size_t), ("fordblks", ctypes.c_size_t),
...
# Test: Clear cache reference after backward (let Python GC handle tensor cleanup)
for i in range(200):
    out = layer.forward(x)
    ttnn.synchronize_device(device)
    grad_out = ttnn.from_torch(torch.randn(B, T, D, dtype=torch.bfloat16), dtype=ttnn.bfloat16, layout=ttnn.TILE_LAYOUT, device=device)
    ...
    _safe_deallocate(out); _safe_deallocate(grad_out); _safe_deallocate(grad_x)
    for g in grads.values(): _safe_deallocate(g)
    # Just drop the cache reference - Python GC + ttnn destructor will handle cleanup
    layer._cache = None
    gc.collect(); libc.malloc_trim(0)
