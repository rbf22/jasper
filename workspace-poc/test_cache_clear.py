"""Test if explicit deallocation of intermediates fixes the leak."""
...
# Test 1: Normal (no explicit dealloc of intermediates, no cache clear)
for i in range(100):
    out = layer.forward(x)
    ttnn.synchronize_device(device)
    grad_out = ttnn.from_torch(torch.randn(B, T, D, dtype=torch.bfloat16), dtype=ttnn.bfloat16, layout=ttnn.TILE_LAYOUT, device=device)
    ...
    if i in {0, 49, 99}:
        used = heap_used()
        delta = used - used0
        print(f"  [no clear] iter {i:3d}: delta={delta//1024:+d}KB  rate={delta/(i+1)/1024:.3f} KB/iter")

# Reset
gc.collect(); libc.malloc_trim(0)
used0 = heap_used()

# Test 2: Clear cache after backward
for i in range(100):
    out = layer.forward(x)
    ttnn.synchronize_device(device)
    grad_out = ttnn.from_torch(torch.randn(B, T, D, dtype=torch.bfloat16), dtype=ttnn.bfloat16, layout=ttnn.TILE_LAYOUT, device=device)
    ...
    for k, v in layer._cache.items():
        if v is not x and v is not None:
            _safe_deallocate(v)
    layer._cache = None
    gc.collect(); libc.malloc_trim(0)
    if i in {0, 49, 99}:
        used = heap_used()
        delta = used - used0
        print(f"  [clear cache] iter {i:3d}: delta={delta//1024:+d}KB  rate={delta/(i+1)/1024:.3f} KB/iter")
