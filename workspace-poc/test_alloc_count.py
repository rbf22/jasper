"""Test with allocation counter to track malloc/free imbalance."""
...
# Load the allocation counter library
counter = ctypes.CDLL("/tmp/leak_counter.so")
counter.log_alloc_state.argtypes = [ctypes.c_char_p]
counter.log_alloc_state.restype = None
counter.log_aligned_sizes.argtypes = [ctypes.c_char_p]
counter.log_aligned_sizes.restype = None

def log_state(label):
    counter.log_alloc_state(label.encode())
    counter.log_aligned_sizes(label.encode())
...
# Run 100 iterations, logging alloc state every 10
for i in range(100):
    out = layer.forward(x)
    ttnn.synchronize_device(device)
    grad_out = ttnn.from_torch(torch.randn(B, T, D, dtype=torch.bfloat16), dtype=ttnn.bfloat16, layout=ttnn.TILE_LAYOUT, device=device)
    ...
    if i % 10 == 9:
        ...
        ctypes.CDLL("libc.so.6").malloc_trim(0)
        rss = rss_kb()
        log_state(f"iter_{i+1}")
        print(f"  iter {i+1:3d}: RSS={rss//1024}MB  delta={((rss-rss0)//1024):+d}MB", flush=True)

gc.collect()
ctypes.CDLL("libc.so.6").malloc_trim(0)
rss1 = rss_kb()
log_state("final")
print(f"After 100 iters: RSS={rss1//1024}MB (delta={((rss1-rss0)//1024):+d}MB)", flush=True)
