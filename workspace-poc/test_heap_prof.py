"""Test with gperftools heap profiler."""
...
# Get the heap profiler functions
profiler = ctypes.CDLL("libprofiler.so.0")
profiler.HeapProfilerStart(b"/tmp/heap_prof")
profiler.HeapProfilerDump.argtypes = [ctypes.c_char_p]
profiler.HeapProfilerDump.restype = None
...
rss0 = rss_kb()
profiler.HeapProfilerDump(b"baseline")
...
ctypes.CDLL("libc.so.6").malloc_trim(0)
rss1 = rss_kb()
profiler.HeapProfilerDump(b"after_20_iters")
print(f"After 20 iters: RSS={rss1//1024}MB (delta={((rss1-rss0)//1024):+d}MB)", flush=True)
