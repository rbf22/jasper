...
\textbf{Important:} All results reported below for Cells B and C were obtained \emph{before} the causal masking fix (see \Cref{sec:workspace}). The workspace's cross-attention was bidirectional, allowing future-token leakage. These results are therefore invalid and are retained only for historical context. Retraining with the fix is in progress.

\paragraph{Cells A and B: completed (results invalid for Cell B).}
Cell A (backbone-only control, 10.08M parameters) early-stopped at step 7608 with a best loss of 0.931. Cell A has no workspace and is unaffected by the causal masking bug. Cell B (backbone + workspace, 10.53M parameters) early-stopped at step 9404 with a best loss of 0.083. \emph{Cell B's results are invalid}: the bidirectional workspace allowed answer tokens to leak into the prediction positions, so the low loss may reflect answer copying rather than genuine reasoning. Cell B is being retrained with the causal masking fix.
