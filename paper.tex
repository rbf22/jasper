...
The more substantive question is whether the workspace architecture provides enough \emph{computational leverage} to matter at scale. The current 30M-parameter model shows promising signs (Task 2 at 85\%, surpassing the Cell E control's 80\%), but Task 1 (multi-hop reasoning) has not yet emerged. This may simply require more training steps---Cell E needed 600+ steps to reach its current level, and the workspace cells are still early in training. Alternatively, the workspace may need to be larger (more slots, wider projections) to handle the compositional structure of multi-hop reasoning, which would reduce its efficiency advantage over simply scaling up the backbone.
