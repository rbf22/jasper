...
Training is currently in progress on Tenstorrent Quietbox 2 hardware (Blackhole architecture, bfloat16-native). The following preliminary observations are from the first $\sim$300--500 training steps:

\paragraph{Convergence speed.}
Cell C (workspace, no recurrence) shows dramatically faster loss reduction than Cell B (hybrid baseline) at matched step counts, but is prone to gradient instability at higher learning rates. At $\text{lr}=6 \times 10^{-4}$, Cell C's gradient norms exploded (norm $\sim$730) after reaching loss 0.18 at step 250. Reducing to $\text{lr}=2 \times 10^{-4}$ stabilized training. This suggests the workspace accelerates learning but creates a sharper loss landscape.

\paragraph{Task accuracy (early).}
At step 300, Cell B achieves 56.7\% overall accuracy, with strong performance on Task 3 (95\%) and Task 2 (75\%), but 0\% on Task 1 (multi-hop chain reasoning). This is expected: without a workspace, the model cannot maintain intermediate variable values across a chain of dependencies. Cells C and D have not yet reached sufficient training steps for meaningful accuracy evaluation.

\paragraph{Backward pass verification.}
The workspace backward pass has been verified against PyTorch autograd on CPU. All gradients---input gradients, weight gradients, slot gradients, and gate gradients---match to within float32 precision ($<10^{-4}$ relative error). The gradient explosion was confirmed to be a training dynamics issue, not a backward pass bug.
