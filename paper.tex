...
\paragraph{Cell C converges faster despite a 3$\times$ lower learning rate.}
Cell C (workspace, no recurrence, lr=$2 \times 10^{-4}$) shows dramatically faster loss reduction than Cell B at matched step counts. At step 150, Cell C's loss (1.34) is already approaching Cell B's plateau loss ($\sim$1.03), despite using a 3$\times$ lower learning rate. The workspace appears to accelerate learning by providing a structured substrate for intermediate representations. However, Cell C is prone to gradient instability at higher learning rates: at lr=$6 \times 10^{-4}$, gradient norms exploded (norm $\sim$730) after reaching loss 0.18 at step 250. Reducing to lr=$2 \times 10^{-4}$ stabilized training, suggesting the workspace creates a sharper loss landscape.

\paragraph{Cell C's first eval (step 100) shows 0\% Task 1.}
At step 100 (still in warmup, lr=$1 \times 10^{-4}$), Cell C achieves 23\% overall accuracy: 0\% Task 1, 60\% Task 2, 10\% Task 3. This is too early to draw conclusions---the model has not yet reached its full learning rate. The critical test will be at steps 200--500, once the LR has warmed up and the workspace has had time to learn the multi-hop dependency tracking.

\paragraph{Cell D is progressing but slow.}
Cell D (full architecture, recurrent core) is at step 47 with loss 3.01, down from 4.91 at step 1. The recurrent core makes each step $\sim$2.5$\times$ slower ($\sim$160s/step vs $\sim$65s/step for B/C), and the 200-step warmup means the model is still far from its peak learning rate. The first checkpoint (step 100) is expected $\sim$2 hours from the time of writing.

\paragraph{Cell E launched as learning-rate control.}
Cell E (B architecture, C/D learning rate of $2 \times 10^{-4}$) has been launched on the device freed by Cell B. This is the critical control for isolating the workspace effect: if Cell C beats Cell E on Task 1, the workspace is doing real work. If Cell E matches Cell C, then Cell B's failure on Task 1 was a learning-rate artifact, not an architectural limitation. Cell E is currently in early warmup (step 2, lr=$2 \times 10^{-6}$).
...
\bottomrule
\end{tabular}
\caption{Evaluation results from the automated eval loop (5 examples per task per depth, depths 2/4/6/8). Task 1 is multi-hop chain reasoning---the target metric for workspace benefit. Cell B has been evaluated at 4 checkpoints with 0\% Task 1, confirming plateau.}
...
The gradient instability observed in Cell C is a concern: if the workspace creates a sharper loss landscape, training at scale may require careful hyperparameter tuning or architectural modifications (e.g., gradient clipping, lower learning rates, or slot normalization schedules) that limit the practical benefit.
