...
\paragraph{Three stabilization techniques.}
The workspace and recurrent core introduce three sources of gradient instability that are not present in the baseline hybrid model. We address each with a targeted normalization:
...
Each technique targets a distinct mechanism: LR groups address the workspace's sharper loss landscape, $1/\sqrt{K}$ scaling addresses recurrent gradient accumulation, and slot normalization addresses unbounded parameter growth. All three are deterministic (no new hyperparameters or learnable parameters) and can be disabled independently for ablation.
...
\paragraph{Workspace gradient instability and the three stabilization techniques.}
...
We identified three distinct instability mechanisms and addressed each with a targeted normalization (\Cref{sec:training-config}):
\begin{enumerate}[nosep]
    ...
    \item \textbf{Unbounded slot growth:} The slot parameters grow via a positive feedback loop (large slots $\to$ sharp attention $\to$ large gradients $\to$ larger slots). Slot parameter normalization constrains slots to unit RMS after each optimizer step, breaking the loop.
\end{enumerate}
Cells C and D have been restarted with all three stabilizations applied. Early results (steps 16--20) show stable gradient norms (C: 5.3, D: 9.7), but the critical test---passing step 200 without collapse---is still $\sim$3 hours away at the time of writing.
...
E & 400 & 38\% & 0\% & 75\% & 40\% \\
\midrule
\multicolumn{6}{l}{\textit{Old runs (before stabilization, collapsed at step 200):}} \\
C (old) & 100 & 23\% & 0\% & 60\% & 10\% \\
C (old) & 200 & 7\% & 5\% & 15\% & 0\% \\
D (old) & 100 & 5\% & 0\% & 15\% & 0\% \\
D (old) & 200 & 22\% & 0\% & 60\% & 5\% \\
\midrule
\multicolumn{6}{l}{\textit{New runs (with LR groups + slot norm + $1/\sqrt{K}$):}} \\
C (new) & \multicolumn{5}{l}{in progress (step 16, stable)} \\
D (new) & \multicolumn{5}{l}{in progress (step 20, stable)} \\
\bottomrule
\end{tabular}
\caption{Evaluation results from the automated eval loop (5 examples per task per depth, depths 2/4/6/8). Task 1 is multi-hop chain reasoning---the target metric for workspace benefit. Both B and E show 0\% Task 1 across all evals, confirming the architectural plateau. The old C run collapsed at step 200 (output degeneration); the new runs with stabilization are in early warmup.}
...
The gradient instability observed in initial Cell C training is a concern: the workspace creates a sharper loss landscape than the baseline hybrid model, and the slot parameters are subject to a positive feedback loop that causes unbounded growth. We have addressed this with three targeted normalizations (per-parameter LR groups, $1/\sqrt{K}$ residual scaling, and slot parameter normalization; see \Cref{sec:training-config}), but these fixes are still being validated. If the stabilized training still collapses, or if the normalizations limit the workspace's expressivity at scale, the practical benefit of the architecture may be constrained. The slot normalization in particular constrains the slot magnitude but not its direction---if the useful information requires magnitude variation across slots, this constraint could limit performance.
