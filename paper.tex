...
The gradient instability was a significant concern early in development, but the stabilization journey has been progressively successful: each fix addressed a real, identified mechanism, and each pushed the collapse further out (step 200 $\to$ 300 $\to$ 400 with $C=1$ $\to$ currently stable past all previous failure points with $C=5$). The four stabilizations (per-parameter LR groups, $1/\sqrt{K}$ residual scaling, slot normalization, and spectral norm capping at $C=5$; see \Cref{sec:training-config}) are now sufficient for stable training through peak LR. The remaining risk is narrower: if the $C=5$ cap is still too restrictive for the attention selectivity that complex reasoning requires, the workspace's expressivity may be limited. This is a tunable parameter, not a fundamental flaw---$C$ can be increased if needed, trading off stability margin for capacity.

The more substantive question is whether the workspace architecture provides enough \emph{computational leverage} to matter at scale. The current 30M-parameter model shows promising signs (Task 2 at 85\%, surpassing the Cell E control's 80\%), but Task 1 (multi-hop reasoning) has not yet emerged. Diagnostic inspection revealed that the workspace was in a degenerate state---acting as a global average pool rather than selective memory---which likely explains the absence of Task 1 progress. Attention entropy and slot diversity regularizers were added to push the workspace toward selective behavior, but it remains to be seen whether this is sufficient, or whether the workspace needs to be larger (more slots, wider projections) to handle the compositional structure of multi-hop reasoning. If the regularizers succeed, the workspace cells should begin showing Task 1 accuracy as training progresses; if not, a larger workspace or an auxiliary loss (using known intermediate values from the synthetic tasks) may be needed.
