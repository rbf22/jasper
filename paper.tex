...
\begin{enumerate}[nosep]
    ...
    \item \textbf{Gate freeze} (new): during the first $N_{\text{freeze}}$ steps (3{,}000 for synthetic, 600 for text), workspace gates are held at exactly 0 after each optimizer step via \texttt{freeze\_workspace\_gates()}. This makes the workspace a true identity (no-op) during the freeze, so the backbone trains as if the workspace doesn't exist. After the freeze, gates learn freely at $1\times$ LR. This was added after evaluation showed that even with ReZero initialization, noisy gradients at high gate LR caused gates to drift open before the workspace had learned useful representations, destabilizing the backbone. The freeze ensures a clean B-vs-A comparison: B trains as exactly A during freeze, then the workspace opens only if gradient descent finds it useful.
    ...
    \item \textbf{Restore-on-spike}: if the pre-clip gradient norm exceeds 5000, the model and optimizer state are restored from the last checkpoint rather than skipping the step. Skip-on-spike freezes the model in the bad state that caused the spike --- it can never recover. Restore gives a fresh start from a known-good state (at most 100 steps old).
\end{enumerate}
...
\paragraph{Evaluation of pre-restart checkpoints.}

After fixing the evaluator (TT-Metal's batch-size-1 matmul limitation was handled by padding to batch 4), all three cells were evaluated on the synthetic tasks before the v2 restart:
...
\toprule
\textbf{Cell} & \textbf{Step} & \textbf{Task 1} & \textbf{Task 2} & \textbf{Task 3} & \textbf{Overall} & \textbf{Depth 2} & \textbf{Depth 8} \\
\midrule
...
\bottomrule
\end{tabularx}
\caption{Evaluation of pre-restart checkpoints. Task 1 = chained assignment arithmetic (multi-hop), Task 2 = permutation tracking (state-tracking), Task 3 = single-hop recall (control). None of the cells learned Task 1. Cell B (workspace, no recurrence) performed \emph{worse} than Cell A (control), indicating the workspace actively destabilized the backbone.}
\label{tab:eval-prerestart}
\end{table}

Key findings:
\begin{itemize}[nosep]
    \item \textbf{None of the cells learned Task 1} (chained assignment arithmetic), even Cell A at step 9600. The multi-hop reasoning task is beyond what a 10M retention model can learn from synthetic character-level data at this scale.
    \item \textbf{Cell B is worse than Cell A} on Tasks 2 and 3. The workspace actively hurt the backbone: gradient norms were 250--2100 vs A's $\sim$8, and 12\% of training steps were wasted on restore/skip events. The 3$\times$ gate LR caused gates to drift open via noisy gradients before the workspace had learned useful representations.
    \item \textbf{Cell C is better than Cell B} (27.5\% vs 15.8\% overall) with stable gradients (0 restores), but still worse than Cell A on Tasks 2/3. The recurrent core appears to mediate the workspace's gradient amplification, but not enough to overcome the backbone destabilization.
    \item \textbf{Teacher-forced accuracy matches free-generation accuracy} for all cells, confirming the failures are reasoning failures, not decoding artifacts.
\end{itemize}

\paragraph{Gate freeze and v2 restart.}

The evaluation showed that the workspace was actively harming the backbone rather than merely failing to help. The root cause was premature gate opening: even with ReZero initialization (gates start at 0), the 3--10$\times$ gate LR caused gates to drift open due to noisy gradients before the workspace had learned useful representations.

The fix adds a \textbf{gate freeze period}: for the first $N_{\text{freeze}}$ steps, workspace gates are held at exactly 0 after each optimizer step. The workspace is a true no-op (identity) during freeze, so the backbone trains as if the workspace doesn't exist --- Cell B becomes exactly Cell A. After the freeze, gates learn freely at $1\times$ LR (reduced from 3--10$\times$). This ensures a clean B-vs-A comparison: if B matches A during freeze and the workspace opens but doesn't help after unfreeze, that supports the conclusion that workspace alone is insufficient.

Cells B and C were restarted from scratch with gate freeze (3{,}000 steps) and 1$\times$ gate LR. At initialization, both show gradient norms $\sim$8.8 matching Cell A, confirming the workspace is a true no-op during freeze.

\paragraph{Cell A: completed.}
Cell A (backbone-only control, 10.08M parameters) completed training at step 9{,}999 with final loss 0.83. It mastered Tasks 2 and 3 (97.5\% and 100\%) but scored 0\% on Task 1, confirming that the multi-hop reasoning task is beyond the backbone's capacity at this scale --- the workspace and recurrent core must provide the missing capability for the architecture to succeed.

\paragraph{Cells B v2 and C v2: in progress.}
Both cells are training from scratch with gate freeze. As of this writing, both are in the freeze period (gates at 0.0, gradient norms $\sim$8.8, matching Cell A's trajectory). The critical observation point is step 3{,}000 when gates unfreeze: if B v2 matches A on Tasks 2/3 at that point, the clean baseline is established. If the workspace then opens and B v2 does not improve, that supports the ``workspace alone is insufficient'' hypothesis. Cell C v2 tests whether the recurrent core provides the missing ingredient.

\paragraph{Text training: tiny challenges corpus.}

In parallel with the synthetic POC, a text training run is testing the architecture on a 2M-example mixed reasoning corpus (``tiny challenges''), inspired by Enigmata-style synthetic reasoning training. The corpus mixes narrative logic puzzles (40\%), BrainBashers-style attribute/elimination puzzles (55\%), and bAbI QA (5\%), for 78.7M training tokens. The shift from TinyStories to tiny challenges reflects the finding that the synthetic POC Task 1 was not learnable at 10M parameters --- the text corpus tests whether framing multi-hop reasoning as text continuation is more learnable.

The text model (29.8M parameters including 19.3M embedding) trains with gate freeze for 600 steps, $K_{\max}=6$, seq\_len=512, effective batch 384, at $\sim$112\,s/step ($\sim$62 hours for 2{,}000 steps). Initial loss is 10.92 ($\approx \log 50257$, confirming correct random init), gradient norm 1.5, gates at 0.0.

% ══════════════════════════════════════════════════════════════════════════
\subsection{Text MVP: Simplified Latent-Memory Architecture}
\label{sec:text-mvp}
% ══════════════════════════════════════════════════════════════════════════

After the synthetic POC showed that the full WRAP architecture (retention backbone + workspace + attention residuals) was too complex to train at 10M parameters on character-level tasks, we developed a simplified \textbf{text latent-memory} architecture that preserves the core idea --- fixed-size latent memory with recurrent reasoning --- while removing the components that caused the most engineering friction (retention layers, QK normalization, ReZero gates, attention residuals, spectral normalization, slot parameter normalization). The goal is to validate the central hypothesis --- that fixed-size latent memory with recurrent refinement can learn multi-hop reasoning from text --- at minimal architectural complexity, then re-introduce WRAP-specific components only if the simplified version succeeds.

\paragraph{Architecture.}

The text MVP is a standard transformer encoder-decoder with a fixed-size latent memory and recurrent transition:

\begin{enumerate}[nosep]
    \item \textbf{Encoder}: $L_{\text{enc}}$ transformer encoder layers (pre-norm, GELU FFN) encode the prompt into hidden states $\mathbf{X} \in \R^{B \times T \times d}$.
    \item \textbf{Memory initialization}: $m$ learnable slot queries $\mathbf{Q} \in \R^{m \times d}$ cross-attend to the encoded prompt, producing initial memory $\mathbf{M}_0 \in \R^{B \times m \times d}$. This is a single Perceiver-IO-style read, without QK normalization or ReZero gates.
    \item \textbf{Recurrent transition}: $K$ steps of a shared transition block update the memory in-place. Each step applies slot-wise self-attention, a SiLU-gated FFN, and a sigmoid write gate:
    \begin{align}
        \mathbf{H} &= \text{RMSNorm}(\mathbf{M}_k) + \text{SelfAttn}(\text{RMSNorm}(\mathbf{M}_k)) \\
        \mathbf{P} &= \text{FFN}(\mathbf{H}) \quad \text{(SiLU activation)} \\
        \mathbf{G} &= \sigma(\text{Linear}(\mathbf{H})) \quad \text{(per-slot write gate)} \\
        \mathbf{M}_{k+1} &= \text{RMSNorm}(\mathbf{M}_k + \mathbf{G} \odot \mathbf{P})
    \end{align}
    The write gate is initialized open (bias $= 4.0$, giving $\sigma(4) \approx 0.98$), allowing the transition to modify memory from the start. This is the opposite of WRAP's ReZero approach (gates start closed) --- the simplified architecture trusts the transition to learn useful updates without a freeze period.
    \item \textbf{Decoder}: $L_{\text{dec}}$ transformer decoder layers autoregressively generate the answer, cross-attending to the final memory $\mathbf{M}_K$ (not the prompt). The decoder uses a standard causal mask and teacher forcing during training.
    \item \textbf{Output}: weight-tied LM head ($\text{logits} = \mathbf{X}_{\text{dec}} \cdot \mathbf{E}^\top$ where $\mathbf{E}$ is the shared token embedding).
\end{enumerate}

\paragraph{Key differences from WRAP.}
...
\begin{tabularx}{\textwidth}{p{0.22\textwidth} p{0.36\textwidth} p{0.36\textwidth}}
\toprule
\textbf{Component} & \textbf{WRAP (synthetic POC)} & \textbf{Text MVP} \\
\midrule
Backbone & Hybrid retention + attention (13--14 layers) & Standard transformer encoder-decoder \\
Workspace & Perceiver-style with QK-norm, ReZero gates, slot decay & Single cross-attention read, no QK-norm, no ReZero \\
Recurrent core & Attention residuals over $K+1$ outputs & Shared transition block, sigmoid write gate \\
Normalization & Slot RMSNorm + backbone spectral norm & RMSNorm only \\
Gate init & ReZero (closed, $\text{gate}=0$) & Sigmoid (open, $\text{bias}=4$) \\
Gradient stabilization & 6 techniques (QK-norm, ReZero, $1/K$ scaling, AR core, slot norm, spectral norm) & None (relies on simple architecture) \\
Memory & 16 slots, $d=384$ & 8--16 slots, $d=128$ (test) or $d=384$ (full) \\
\bottomrule
\end{tabularx}
\caption{Architectural differences between WRAP and the text MVP. The text MVP removes all six gradient stabilization techniques to test whether they are necessary at the simplified architecture's scale.}
\label{tab:wrap-vs-mvp}
\end{table}

\paragraph{Why simplify.}

The WRAP architecture required six distinct gradient stabilization techniques to train without divergence (see \Cref{sec:training-config}). Each technique adds hyperparameters, constrains the model's expressivity, or complicates the implementation. Before investing in scaling the full WRAP architecture, we want to know whether the \emph{core idea} --- fixed-size latent memory with recurrent refinement --- works at all in the text setting. The text MVP tests this with minimal scaffolding. If the MVP learns multi-hop reasoning from text, it validates the approach and motivates re-introducing WRAP components for scaling. If the MVP fails, the failure is more informative than a WRAP failure because it cannot be attributed to insufficient stabilization.

\paragraph{Native Tenstorrent implementation.}

The text MVP is implemented natively on Tenstorrent hardware using \texttt{ttnn} operations, without PyTorch autograd. The forward and backward passes are hand-written:

\begin{itemize}[nosep]
    \item \textbf{Forward}: encoder/decoder attention, FFN, and embedding lookups use \texttt{ttnn.matmul}, \texttt{ttnn.moreh\_layer\_norm}, and device-side embedding lookup. All intermediates are cached on-device for backward.
    \item \textbf{Backward}: attention backward uses \texttt{ttnn.matmul} and \texttt{ttnn.moreh\_softmax\_backward} on device. Linear backward uses \texttt{ttnn.matmul} on device. RMSNorm and LayerNorm backward are computed on host in float32 for numerical stability (BF16 \texttt{rsqrt} overflows after $\sim$30 steps). Cross-entropy loss is computed on host in float32 (the $V \times V$ identity matrix for on-device softmax is infeasible at $V{=}50257$). Embedding backward uses host-side \texttt{torch.index\_add\_} (scatter-add by token ID).
    \item \textbf{Optimizer}: custom AdamW implementation for TT tensors, with float32 master weights and BF16 device weights.
\end{itemize}

The implementation avoids three critical issues that caused earlier training failures:

\begin{enumerate}[nosep]
    \item \textbf{Device hang after $\sim$100 steps}: caused by variable sequence lengths per batch triggering new TT-Metal kernel compilations. Each unique tensor shape compiles a new kernel; after $\sim$100 shapes, the device stalled at 0\% CPU. Fixed by padding all batches to fixed \texttt{max\_prompt\_len} and \texttt{max\_answer\_len}, so kernels compile once and are reused.
    \item \textbf{RSS growth (284\,MB/100 steps)}: caused by glibc malloc arena retention of large temporary host tensors from cross-entropy loss computation. Fixed with explicit \texttt{del} of temporary tensors and \texttt{malloc\_trim(0)} every 10 steps. RSS now plateaus at $\sim$3.4\,GB after step 40 with zero growth thereafter.
    \item \textbf{Gradient \texttt{inf} (steps 53/54/97/105)}: caused by BF16 \texttt{rsqrt} overflow in RMSNorm/LayerNorm backward (7-bit mantissa insufficient for small variance values). Fixed by moving norm backward to host float32. Combined with fixed sequence lengths (which stabilize attention matmul shapes), no \texttt{inf} gradients occur in 500-step runs.
\end{enumerate}

\paragraph{CPU-vs-TT parity.}

Gradient parity is verified against a PyTorch CPU reference model with identical weights. All 12 compared parameter gradients match within BF16 tolerance (max abs diff $= 0.041$, mean abs diff $= 0.0008$). The TT gradient norm is $1.47\times$ the CPU gradient norm, attributable to BF16 matmuls in the attention backward. The parity test confirmed that the embedding backward --- which was initially skipped (only the LM head path contributed) --- is now correctly accumulated from both the encoder input path and the decoder input path, improving the token embedding gradient match from 0.29 to 0.041 max abs diff.

\paragraph{Training results.}

The text MVP trains stably on the tiny challenges corpus at the test configuration ($d{=}128$, 2 encoder layers, 1 decoder layer, 4 heads, 8 slots, $K{=}3$, 7.3M parameters, BF16):
...
\bottomrule
\end{tabularx}
\caption{Text MVP training on tiny challenges (test config, 500 steps). Loss decreases from $\log(50257) \approx 10.9$ to $\sim$3.0, token accuracy reaches $\sim$48\%, RSS is flat from step 40, no gradient \texttt{inf}, no device hang. 500 steps in 51 seconds (0.1\,s/step).}
\label{tab:text-mvp-results}
\end{table}

The model converges rapidly: loss drops 72\% in 500 steps, token accuracy reaches 48\% (peaking at 56\% at step 420). The gradient norms are stable (3.3--10.9 range) with no \texttt{inf} or \texttt{NaN}. RSS is flat at 3407\,MB from step 40 onward. The full 500-step run completes in 51 seconds.

\paragraph{What this tells us.}

The text MVP demonstrates that:
\begin{enumerate}[nosep]
    \item The core architectural idea --- fixed-size latent memory with recurrent refinement --- trains stably on text without any of WRAP's six gradient stabilization techniques.
    \item Native Tenstorrent training (forward + backward + optimizer) is feasible and fast (0.1\,s/step at 7.3M parameters, BF16).
    \item The model learns from the tiny challenges corpus, suggesting that text-framed multi-hop reasoning is more learnable than the character-level synthetic tasks.
\end{enumerate}

What it does \emph{not} yet tell us:
\begin{enumerate}[nosep]
    \item Whether the model can solve \emph{specific} multi-hop reasoning tasks (we measure token accuracy, not task accuracy).
    \item Whether the recurrent transition is actually doing multi-step reasoning or just smoothing the memory representation.
    \item Whether the architecture scales to the full $d{=}384$, 13-layer configuration.
    \item Whether the workspace components (QK-norm, ReZero gates, attention residuals) are needed at scale or are artifacts of the retention backbone's instability.
\end{enumerate}

The next step is to scale to the full production configuration ($d{=}384$, 13 layers, 16 slots, $K{=}6$) and evaluate on held-out reasoning tasks with verifiable answers.
...
A sixth fix --- \textbf{gate freeze} --- was added after evaluation showed that even with all gradient instability resolved, premature gate opening at high gate LR destabilized the backbone before the workspace could learn useful representations.
...
The more substantive question is whether the workspace provides enough \emph{computational leverage} to matter. The pre-restart evaluation gave a discouraging signal: Cell B (workspace, no recurrence) performed \emph{worse} than Cell A (control) on Tasks 2 and 3, with 12\% of training steps wasted on gradient spike recovery. The workspace was actively destabilizing the backbone rather than helping it. However, this was with premature gate opening at high gate LR --- the v2 runs with gate freeze will provide a fairer test. If B v2 matches A during freeze and the workspace opens but doesn't help after unfreeze, that cleanly establishes that workspace alone is insufficient. The R1--R4 criteria (\Cref{sec:experiments}) are designed to distinguish these explanations.
...
\paragraph{The text MVP pivot.}

The text MVP (\Cref{sec:text-mvp}) represents a deliberate strategic pivot from the full WRAP architecture. Rather than continuing to debug the six gradient stabilization techniques, we stripped the architecture down to its core idea --- fixed-size latent memory with recurrent refinement --- and asked whether it can learn from text at all. The answer so far is yes: the model trains stably, converges rapidly (loss 10.9 $\to$ 3.0 in 500 steps), and reaches 48\% token accuracy on the tiny challenges corpus, all without any gradient stabilization beyond gradient clipping at norm 1.0.

This does not prove the architecture can solve multi-hop reasoning tasks --- token accuracy on a mixed corpus is a weak signal. But it does establish a stable baseline from which to scale. The next milestone is the full production configuration ($d{=}384$, 13 layers, 16 slots, $K{=}6$) with task-level evaluation on held-out reasoning problems. If the full model can solve specific multi-hop tasks (e.g., the narrative logic puzzles in the tiny challenges corpus), that validates the approach and motivates re-introducing WRAP components for scaling. If it cannot, the failure isolates the question: is the limitation in the memory mechanism, the recurrent transition, or the training scale?

The retention coincidence may be more metaphorical than functional. The retention state and the \jspace{} have similar shapes, but they operate at different levels of abstraction---the retention state is a low-level sequence summary, while the \jspace{} is a high-level conceptual workspace. Whether the workspace module can bridge this gap is an empirical question. The text MVP sidesteps this question by using standard attention (not retention) for the backbone, and testing whether the workspace concept alone --- independent of the retention substrate --- is sufficient for reasoning.
