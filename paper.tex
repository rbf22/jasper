...
\begin{enumerate}[nosep]
    ...
    \item \textbf{Unbounded attention weight growth:} Even with slot magnitudes constrained, the $W_Q$ and $W_K$ projections can grow, producing sharp softmax distributions that cause the same gradient concentration. Attention logit clamping ($C=5$) directly limits the softmax sharpness regardless of weight magnitudes.
\end{enumerate}
The first three stabilizations were sufficient to delay the collapse (from step 200 to step 300 for Cell C) but not prevent it. The fourth---logit clamping---targets the remaining growth path (attention weights) and is currently being tested. Cells C and D have been resumed from their step 200 checkpoints (the last stable point) with all four stabilizations applied.
...
The gradient instability observed in Cell C training is a significant concern. The workspace creates a sharper loss landscape than the baseline hybrid model, and we have identified four distinct instability mechanisms (workspace gradient dominance, recurrent gradient accumulation, unbounded slot growth, and unbounded attention weight growth). The first three stabilizations (per-parameter LR groups, $1/\sqrt{K}$ residual scaling, and slot parameter normalization; see \Cref{sec:training-config}) were sufficient to delay the collapse from step 200 to step 300 but not prevent it---the attention weight matrices continued to grow even with slot magnitudes constrained. A fourth stabilization (attention logit clamping) directly limits softmax sharpness and is currently being tested. If this also fails, or if the normalizations collectively limit the workspace's expressivity at scale, the practical benefit of the architecture may be constrained. The slot normalization constrains slot magnitude but not direction, and the logit clamp limits attention sharpness to $e^{10}:1$---if useful reasoning requires sharper attention or magnitude variation across slots, these constraints could limit performance.
