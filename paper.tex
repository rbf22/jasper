...
\midrule
\multicolumn{6}{l}{\textit{With all 4 stabilizations, spectral norm capped at $C=5$ + regularizers (collapsed at step 450):}} \\
C (reg) & 400 & --- & --- & --- & --- \\
C (reg) & \multicolumn{5}{l}{loss dropped to 0.55 (best ever), write entropy 2.75 $\to$ 0.224, then collapsed at step 450} \\
\midrule
\multicolumn{6}{l}{\textit{With architectural fix (zero-init gates + slot decay, fresh start, in progress):}} \\
C (arch) & \multicolumn{5}{l}{training, loss 4.90 $\to$ 1.96 in 50 steps (pre-reboot), stable} \\
D (arch) & \multicolumn{5}{l}{training, loss 4.94 $\to$ 3.31 in 38 steps (pre-reboot), stable} \\
\bottomrule
\end{tabular}
\caption{Evaluation results from the automated eval loop (5 examples per task per depth, depths 2/4/6/8). Task 1 is multi-hop chain reasoning---the target metric for workspace benefit. Both B and E show 0\% Task 1 across all evals (E's 5\% at step 600 is likely noise---1/20 examples), confirming the architectural plateau. The old C run collapsed at step 200; with 3 stabilizations it passed step 200 but collapsed at step 300. Spectral norm at $C=1$ passed the old collapse points (C at step 300: 33\% overall, 85\% Task 2---best ever for C) but re-collapsed at peak LR due to the capacity mismatch. With $C=5$ + regularizers, the workspace learned selective attention (write entropy 2.75 $\to$ 0.224) and the loss dropped to 0.55, but the model collapsed at step 450 due to the structural feedback loop instability. The architectural fix (zero-init gates + slot decay) is currently training with a 3,000-step budget.}
