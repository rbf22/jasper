...
\paragraph{Attention logit clamping.}
Even with slot normalization, the attention weight matrices ($W_Q, W_K$) can grow during training, producing large attention logits $\mathbf{Q}\mathbf{K}^\top / \sqrt{d_h}$ that create sharp softmax distributions. Sharp softmax has exponentially small gradients for non-attended positions, causing gradient concentration and instability. To prevent this, attention logits are clamped to $[-C, C]$ before softmax in both the read and write phases:
\begin{equation}
    \mathbf{A} = \text{softmax}\!\left(\text{clamp}\!\left(\frac{\mathbf{Q}\mathbf{K}^\top}{\sqrt{d_h}},\; -C,\; C\right)\right)
\end{equation}
with $C = 5$. This limits the maximum attention ratio between any two positions to $e^{2C} \approx e^{10} \approx 22{,}000$:1---more than sufficient for selective attention, but preventing the $e^{100+}$ ratios that arise from unbounded logits. The clamp is differentiable: gradients pass through where $|\text{logit}| \leq C$ and are zeroed where clamped, so the model can still learn to sharpen attention up to the clamp limit.

\paragraph{Spectral normalization of workspace weights.}
While logit clamping caps the symptom (sharp softmax), it does not address the root cause: the weight matrices $W_Q, W_K, W_V, W_O$ can grow without bound, and the clamp only limits the output, not the weights themselves. To address this, we apply spectral normalization to all eight workspace weight matrices after each optimizer step. Each matrix is divided by its spectral norm (largest singular value $\sigma_{\max}$), constraining the Lipschitz constant to 1:
\begin{equation}
    W \leftarrow \frac{W}{\sigma_{\max}(W)}, \quad \sigma_{\max}(W) = \text{largest singular value of } W
\end{equation}
This is the technique that stabilized GAN discriminators \cite{miyato2018spectral}, which face the same problem: unbounded weights $\to$ sharp distributions $\to$ gradient instability. With spectral norm 1 on all projections, unit-RMS slots, and LayerNorm on the input, the attention logits are bounded by construction:
\begin{equation}
    |\text{logit}_{ij}| \leq \|W_Q\|_2 \|W_K\|_2 \|\text{slots}\| \|x\| / \sqrt{d_h} \leq 1 \cdot 1 \cdot 1 \cdot 1 / \sqrt{d_h}
\end{equation}
The model can still learn the \emph{direction} of each weight matrix (which determines what it attends to), just not the \emph{magnitude} (which determines attention sharpness). We compute $\sigma_{\max}$ via 10 steps of power iteration on-device, avoiding host transfers.
...
\paragraph{Five stabilization techniques.}
The workspace and recurrent core introduce five sources of gradient instability that are not present in the baseline hybrid model. We address each with a targeted normalization:
...
\begin{enumerate}[nosep]
    ...
    \item \textbf{Attention logit clamping} (\Cref{sec:workspace}): clamps attention logits to $[-5, 5]$ before softmax, preventing unbounded attention sharpness from growing weight matrices even when slot magnitudes are constrained.
    \item \textbf{Spectral normalization} (\Cref{sec:workspace}): constrains all eight workspace weight matrices to spectral norm 1 after each optimizer step, addressing the root cause of weight growth directly rather than capping the resulting logits.
\end{enumerate}

Each technique targets a distinct mechanism: LR groups address the workspace's sharper loss landscape, $1/\sqrt{K}$ scaling addresses recurrent gradient accumulation, slot normalization addresses unbounded slot parameter growth, logit clamping caps the symptom (sharp softmax), and spectral normalization addresses the root cause (unbounded weight matrices). All five are deterministic (no new hyperparameters or learnable parameters beyond the fixed clamp value $C=5$) and can be disabled independently for ablation.
...
\paragraph{Workspace gradient instability and the five stabilization techniques.}
...
We identified five distinct instability mechanisms and addressed each with a targeted normalization (\Cref{sec:training-config}):
\begin{enumerate}[nosep]
    ...
    \item \textbf{Unbounded attention weight growth (symptom):} Even with slot magnitudes constrained, the $W_Q$ and $W_K$ projections can grow, producing sharp softmax distributions. Attention logit clamping ($C=5$) directly limits the softmax sharpness regardless of weight magnitudes---a symptom-level fix.
    \item \textbf{Unbounded attention weight growth (cause):} Spectral normalization constrains all eight workspace weight matrices to spectral norm 1 after each optimizer step, addressing the root cause. With spectral norm 1, unit-RMS slots, and LayerNorm on inputs, attention logits are bounded by $1/\sqrt{d_h} \approx 0.07$ by construction---no clamping needed.
\end{enumerate}
The first three stabilizations delayed the collapse (from step 200 to step 300 for Cell C) but did not prevent it. The fourth (logit clamping) caps the symptom. The fifth (spectral normalization) addresses the root cause and is the current approach. Cells C and D have been resumed from their step 200 checkpoints (the last stable point) with all five stabilizations applied.
...
The gradient instability observed in Cell C training is a significant concern. The workspace creates a sharper loss landscape than the baseline hybrid model, and we have identified five distinct instability mechanisms (workspace gradient dominance, recurrent gradient accumulation, unbounded slot growth, unbounded attention weight growth at the logit level, and unbounded attention weight growth at the weight level). The first three stabilizations (per-parameter LR groups, $1/\sqrt{K}$ residual scaling, and slot parameter normalization; see \Cref{sec:training-config}) were sufficient to delay the collapse from step 200 to step 300 but not prevent it---the attention weight matrices continued to grow even with slot magnitudes constrained. A fourth stabilization (attention logit clamping) caps the resulting softmax sharpness, and a fifth (spectral normalization) constrains the weight matrices directly, addressing the root cause. If spectral normalization also fails, or if the normalizations collectively limit the workspace's expressivity at scale, the practical benefit of the architecture may be constrained. Spectral norm 1 is a strong constraint---if useful reasoning requires weight matrices with larger spectral norms, this constraint could limit performance.
