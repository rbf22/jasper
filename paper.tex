...
\paragraph{The entropy regularizer is actively harmful: entropy is the wrong objective.}
With the entropy and diversity regularizers active (weight 0.01 each), Cell C's write attention entropy fell from 2.75 to 0.224 (out of $\ln 16 \approx 2.77$ for uniform) and the answer-token loss reached 0.55---the lowest we have recorded, against 0.94 for the plateaued control. We initially read this as the workspace learning selective memory. Direct evaluation of that checkpoint shows the opposite.
...
\paragraph{The Task 1 failure is a reasoning failure, not exposure bias.}
...
\bottomrule
\end{tabular}
\caption{Evaluation results from the automated eval loop (5 examples per task per depth unless noted, depths 2/4/6/8). Task 1 is multi-hop chain reasoning---the target metric for workspace benefit. \textbf{No configuration has ever exceeded noise on Task 1}, with or without a workspace. B and E both plateau at 0--5\% Task 1 (E's 5\% is 1/20 examples), confirming an architectural limit that the workspace has not yet lifted. The old C run collapsed at step 200; with 3 stabilizations it passed step 200 but collapsed at step 300. Spectral norm at $C=1$ gave C its best result to date (step 300: 33\% overall, 85\% Task 2, still 0\% Task 1). Adding the entropy/diversity regularizers then produced the \emph{lowest} answer-token loss (0.55) together with the \emph{worst} evaluation (12\% overall, 28\% Task 2), because the write path collapsed to a routing that is identical for 98.3\% of positions across all inputs; those regularizers are now disabled. The workspace's entire measured advantage over the control, at its historical best, is 85\% versus 80\% on Task 2---one task, five points, one seed.}
...
The more substantive question is whether the workspace provides enough \emph{computational leverage} to matter, and here we must be blunt: \textbf{to date there is no evidence that it does.} Across every configuration we have trained---unstabilized, three stabilizations, spectral norm $C=1$, $C=5$, and $C=5$ with regularizers---Task 1 accuracy has never exceeded noise, for workspace and non-workspace cells alike. The workspace's entire measured advantage over the control at its historical best is 85\% versus 80\% on Task 2: one task, five points, a single seed, no error bars. Teacher-forcing rules out decoding as the culprit (22.4\% per-token against a 10\% baseline), so the failure is in the computation.
...
Two possibilities remain open. The first is that our experimental design tests the wrong thing: Task 1 requires iteration \emph{and} memory, and Cell C supplies memory without iteration, so C-versus-E may be architecturally incapable of showing a benefit regardless of workspace quality. On this reading the informative comparison is D-versus-C, and we have under-prioritized D because it is slower and less stable. The second is that the workspace as designed is simply not the right mechanism. It requires eight weight matrices, two gates, a decay term, two normalizations, and three learning-rate groups to train at all; that quantity of scaffolding is itself evidence against the design, since the architectures that won in practice---transformers included---won by being simple and trainable rather than by being optimal. Accordingly we have set an advance stopping rule: if Cell D shows Task 1 $\leq 10\%$ by step 1{,}500 with gates verifiably open and attention verifiably input-dependent, we report the negative result rather than continue to search for one more fix.
