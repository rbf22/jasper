...
Even with slot normalization, the attention weight matrices ($W_Q, W_K, W_V, W_O$) can grow during training, producing large attention logits that create sharp softmax distributions. To address this root cause directly, we apply spectral normalization to all eight workspace weight matrices after each optimizer step. Each matrix is divided by its spectral norm (largest singular value $\sigma_{\max}$), constraining the Lipschitz constant to 1:
\begin{equation}
...
\begin{enumerate}[nosep]
    ...
    \item \textbf{Spectral normalization} (\Cref{sec:workspace}): constrains all eight workspace weight matrices to spectral norm 1 after each optimizer step, addressing the root cause of unbounded weight growth directly.
\end{enumerate}

Each technique targets a distinct mechanism: LR groups address the workspace's sharper loss landscape, $1/\sqrt{K}$ scaling addresses recurrent gradient accumulation, slot normalization addresses unbounded slot parameter growth, and spectral normalization addresses unbounded weight matrix growth. All four are deterministic (no new hyperparameters or learnable parameters) and can be disabled independently for ablation.
...
\begin{enumerate}[nosep]
    ...
    \item \textbf{Unbounded attention weight growth:} Even with slot magnitudes constrained, the $W_Q$ and $W_K$ projections can grow, producing sharp softmax distributions. We initially tried attention logit clamping ($C=5$) to cap the resulting softmax sharpness, but this was a symptom-level fix---the weights continued to grow, and Cell D still showed escalating gradient spikes (10.8 $\to$ 13.7). Spectral normalization constrains all eight workspace weight matrices to spectral norm 1 after each optimizer step, addressing the root cause. With spectral norm 1, unit-RMS slots, and LayerNorm on inputs, attention logits are bounded by $1/\sqrt{d_h} \approx 0.07$ by construction.
\end{enumerate}
The first three stabilizations delayed the collapse (from step 200 to step 300 for Cell C) but did not prevent it. The fourth (spectral normalization) addresses the root cause and is the current approach. Early results with spectral norm show Cell D's gradient spikes dampening (9.7 $\to$ 2.9) instead of escalating (10.8 $\to$ 13.7), and Cell C remaining flat at $\sim$0.9. Cells C and D have been resumed from their step 200 checkpoints with all four stabilizations applied.
